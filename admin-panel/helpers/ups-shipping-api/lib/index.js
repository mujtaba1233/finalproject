...
function UPS(args) {
  ...
  $scope.handleRatesResponse = function(json, callback) {
    ...
    if(json.RatingServiceSelectionResponse.RatedShipment instanceof Array) {
      // console.log('fine',json.RatingServiceSelectionResponse.Response.ResponseStatusDescription);
      
      if(json.RatingServiceSelectionResponse.RatedShipment.length === 0) {
      ...
      for(var i = 0; i < json.RatingServiceSelectionResponse.RatedShipment.length; i++) {
        var j = json.RatingServiceSelectionResponse.RatedShipment[i];
        // if(j.RatingServiceSelectionResponse.Response.ResponseStatusCode !== '1') {
        //   err = j;
        //   continue;
        // }
        if(ret.Response === null) {
          ret.Response = json.RatingServiceSelectionResponse.Response
        }
        ret.RatedShipment.push(j);
      }
  ...
  function doRequest(params, body, callback) {
    ...
    req.on('response', function(res) {
      ...
      res.on('end', async function() {
        var json = ''
        try {
          var json =  await parser.parseStringPromise(responseData);
          // console.log(json);
